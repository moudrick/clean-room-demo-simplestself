...
`node demo.mjs recreate --confirm <LD_PROJECT_KEY value>` uses only reset tokens. It performs reset-token read checks, prints every exact target, requires the exact configured-project confirmation string, deletes only existing disposable targets, creates three public repositories, commits synthetic source, and creates three boolean flags. `demo-checkout-rollout` is in two files in two repositories with recent synthetic commits; `demo-legacy-profile` is in one file with deliberately fabricated old author/committer dates; `demo-retired-banner` is absent. It never deletes configured containers.
...
`node demo.mjs destroy --confirm <LD_PROJECT_KEY value>` uses only reset tokens, requires exact confirmation, and deletes only the six disposable targets. Missing targets are reported as already absent.
