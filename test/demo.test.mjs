...
import { ORG_ENV, PROJECT_ENV, REPOS, FLAGS, ENVIRONMENTS, GH, LD, SOURCES, request, rateLimitDelayMs, doctor, recreate, destroy, checkLaunchDarkly, createRepositoryWithSource, createProject, configureFlagTargeting, removeIfPresent, waitForRepositoryAbsence, waitForProjectAbsence, settingsFor, assertScope, tokensFor, requireConfirmation, outcome, redact } from '../lib.mjs';
...
test('repository provisioning creates an initial commit before its dated synthetic evaluator commit', async () => {
  const calls = []; const responses = [{ default_branch: 'main' }, { object: { sha: 'initial-commit' } }, { tree: { sha: 'initial-tree' } }, { sha: 'blob-package' }, { sha: 'blob-app' }, { sha: 'blob-ignore' }, { sha: 'blob-readme' }, { sha: 'source-tree' }, { sha: 'source-commit' }, {}];
  ...
  const tree = JSON.parse(calls[7].body);
  assert.equal(tree.base_tree, 'initial-tree'); assert.deepEqual(tree.tree.map((entry) => entry.path), ['package.json', 'app.mjs', '.gitignore', 'README.md']);
  ...
  assert.deepEqual(JSON.parse(calls[8].body).parents, ['initial-commit']);
  assert.equal(calls[9].method, 'PATCH'); assert.match(calls[9].url, /git\/refs\/heads\/main$/);
});
...
test('destroy removes the owned project rather than individually deleting flags', async () => {
  ...
  await destroy(fetcher, env, env.LD_PROJECT_KEY);
